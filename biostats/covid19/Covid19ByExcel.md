...
- 無事には開けない場合
  - 日付の欄が「######」のように#の羅列で文字化けしたように見えることがあります。これは列幅が狭くて表示されていないだけで、列幅を広げるときちんと見えるようになります。
- 無事に開ける
...
- 数式をコピーする
  ...
  - 縦に区切ったのは不要なので、右端にでもドラッグして消す。横に区切ったものは上半分はシートの上の方、下半分はシートの下の方（この場合、987行目まであるのでそのあたり）を表示しておくと便利
  - 今、作成したD8セルを選択してコピー。ペーストする先の一番上であるD9セルをクリックすることで選択して、分割した先のD987をShiftキーを押しながらクリックすることで、コピー先を選択した状態にする（破線がくるくるした状態になる）
  - ペーストする（Ctrl-Vなど）。計算結果が出ればOK。
- 7日間平均のグラフを描く
  - 値が入っているセルを全選択する（値が入っているセルを選択したあとでCtrl-A。もしくはA1をクリックして、D987をシフトを押しながらクリック）
